...
while (<>) {
	...
	if ($Check) {
		my ($start,$end,$mid,$triple, $start_codon) = check_CDS($seq);
		print "$key\t$start($start_codon)\t$end\t$mid\t$triple\n" if(!$start || !$end || !$mid || !$triple);
...
sub check_CDS{
	...
	$triple=1 if($len%3 == 0);
	$start=1 if($seq=~/^ATG/);
	my $start_codon=substr($seq, 0, 3); # edited for extracting the fisrt 3 bases;
	...
	return ($start,$end,$mid,$triple, $start_codon);
}
