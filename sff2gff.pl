...
my %genome_length;
open IN, "$in_metadata" || die "$!\n";
while(<IN>){
  chomp;
  my @info=split;
  #print "$info[0]\t$info[1]\n";
  $genome_length{$info[0]} = $info[1];
}
close IN;
...
while(<IN>){
  ...
  if($info[1] eq "exon"){
    ...
    if($info[3] eq "+"){
      ...
      # 1-based coordinates
      $info[4]=$genome_length{$genome_ID}-$info[4]+1;
      $info[6]=$genome_length{$genome_ID}-$info[6]+1;
    }
